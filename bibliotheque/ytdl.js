...
//want more free bot scripts? subscribe to my youtube channel: https://youtube.com/@DGXeon
 ...
 class YT {
     ...
     static getVideoID = (url) => {
         if (!this.isYTUrl(url)) throw new Error('ce n est pas un lien Youtube')
     ...
     static downloadMusic = async (query) => {
         try {
             ...
             let songPath = `./KevFile/audio/${randomBytes(3).toString('hex')}.mp3`
     ...
     static mp4 = async (query, quality = 134) => {
         try {
             if (!query) throw new Error('Necessite une ID Video ou un lien YouTube')
     ...
     static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
         try {
             if (!url) throw new Error('Necessite une ID Video ou un lien YouTube')
