...
const getTime = (format, date) => {
	if (date) {
		...
		return moment.tz('Africa/Abidjan').locale('en-IN').format(format)
	}
}
...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./KevFile/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./KevFile/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./KevFile/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./KevFile/trash/${filename}.mp4`)
    Promise.all([unlink(`./KevFile/trash/${filename}.mp4`), unlink(`./KevFile/trash/${filename}.gif`)])
    return buffer5
}
